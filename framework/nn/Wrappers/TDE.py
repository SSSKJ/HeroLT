...
from BaseModel import BaseModel
from tools.loaders import TDE_loader
...
class TDE(BaseModel):
    ...
    def load_data(
        ...
        if not self.test_mode:
            ...
            splits = ['train', 'val']
            if dataset not in ['inatrual2018', 'imagenet_lt']:
                splits.append('test')
    ...
    def train(self):
        ...
        print(['Force shuffle in training??? --- ', self.do_shuffle])
        ...
        for epoch in range(1, end_epoch + 1):
            for key, model in self.networks.items():
                ...
                if self.config['networks'][key]['optim_params']['lr'] == 0.0:
                    print(['=====> module {} is set to eval due to 0.0 learning rate.'.format(key)])
                    model.eval()
                else:
                    model.train()
    ...
    def __show_current_lr(self):
        ...
        for key, val in self.model_optimizer_dict.items():
            ...
            print('=====> Current Learning Rate of model {} : {}'.format(key, str(lr_set)))
        return max_lr
    ...
    def __batch_backward(self, print_grad=False):
        ...
        if self.training_opt['display_grad']:
            print_grad_norm(self.model_optim_named_params, verbose=print_grad)
        # Step optimizers
        for key, optimizer in self.model_optimizer_dict.items():
            optimizer.step()
        if self.criterion_optimizer:
    ...
    def __saving_feature_with_label_export(self):
        ...
        print('=====> Features with labels are saved as {}'.format(eval_features_dir))
