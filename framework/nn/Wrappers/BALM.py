...
class BALM(BaseModel):
    ...
    def __init__(
        ...
        relatin_opt = self.config['memory']
    ...
    def train(self):
        ...
        print('Do shuffle??? --- ', self.do_shuffle)
        ...
        print('est validation accuracy is %.3f at epoch %d' % (best_acc, best_epoch))
        ...
        if not self.test_mode:
            ...
            if sampler_defs:
                if sampler_defs['type'] == 'ClassAwareSampler':
                    ...
                    self.learner = source_import(sampler_defs['def_file']).get_learner()(
            ...
            self.__training_data = {x: TDE_loader.load_data(data_root=f'{self.base_dir}/datasets/{dataset}',
                                            dataset=dataset, phase=self.__split2phase(x), 
    ...
    def __split2phase(self, split):
        ## todo: add parameter val_as_train
        if split == 'train' and self.config.val_as_train:
            return 'train_val'
        else:
            return split
